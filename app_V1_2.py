...
# ==========================================
DEFAULT_MULTIPLIER = 2.14123e15
DEFAULT_EXPONENT = -9.35714

if "calib_multiplier" not in st.session_state:
    st.session_state["calib_multiplier"] = DEFAULT_MULTIPLIER
if "calib_exponent" not in st.session_state:
    st.session_state["calib_exponent"] = DEFAULT_EXPONENT
                    ...
                    if not valid_sheets:
                        ...
                        if use_subtraction and not solvent_files:
                            ...
                            for s_name, s_file in sample_files.items():
                                ...
                                bio_mg = float(match.iloc[0]["oil_mass_mg"])
                                sol_mg = float(match.iloc[0]["2meth_thf_mass_mg"])
            ...
            if not st.session_state["results_df"].empty:
                ...
                with PdfPages(pdf_buffer) as pdf:
                    ...
                    if st.session_state["fractions_df"] is not None and not st.session_state["fractions_df"].empty:
                        ...
                        pdf.savefig(f_frac_tab, bbox_inches='tight'); plt.close(f_frac_tab)
...
# ==========================================
    ...
    if not st.session_state["all_curves"]:
        ...
        st.write("*(Note: The order you select them determines the left-to-right bar chart grouping)*")
        available_fracs = list(st.session_state["all_curves"].keys())
        selected_fracs = st.multiselect("Select Samples to Chart:", options=available_fracs, default=available_fracs)
        
        fraction_results = []
        for sam_name in selected_fracs:
            if sam_name in st.session_state["all_curves"]:
                data = st.session_state["all_curves"][sam_name]
                MW, W = data['MW'], data['W']
                total_area = np.sum(W)
        ...
        if not st.session_state["fractions_df"].empty:
            st.subheader("Relative Abundance Grid Metric (%)")
            st.dataframe(st.session_state["fractions_df"], hide_index=True)
            
            fig, ax = plt.subplots(figsize=(10, 5))
            colors = ['#4472c4', '#ffc000', '#00b050', '#ed7d31', '#5b9bd5']
            labels = [r[0] for r in mw_ranges]
            x = np.arange(len(st.session_state["fractions_df"]))
            width = 0.15
...
# ==========================================
    ...
    st.header("Step 4: Independent Visual Overlay Screening")
    ...
    with col2:
        ...
        if st.session_state["overlay_dict"] and selected_overlays:
            ...
            fig.savefig(pdf_over_buf, format='pdf', bbox_inches='tight')
            pdf_over_buf.seek(0)
...
# ==========================================
    ...
    The engine pairs uploaded file names (e.g., `PILOT_7_2.csv`) directly with rows in the Oil Concentration file, automatically extracting `oil_mass_mg` and `2meth_thf_mass_mg` metrics to establish proper dilution equations dynamically in memory.
    ...
    * **Baseline Level Alignments:** If subtraction is enabled, aligns timelines via 1D array linear coordinate data interpolation arrays ($np.interp$).
    ...
    * **Integral Averages Estimations:** Fits standard numerical Riemann summations:
    ...
    ### 4. Quick Screening Overlays
    Used for visual shape comparison completely independent of the math engine. It ignores concentration and bio-oil mass. It simply finds the highest raw peak of a sample, and divides every data point by that maximum value. This mathematically forces the highest point of every single curve to hit exactly 1.0 on the Y-Axis.
    """)
